...
export class ValidationService {
  ...
  isControlInvalid(formGroup: FormGroup, controlName: string): boolean {
    ...
    if (isDefined(control?.parent?.errors)) {
      ...
      return isDefined(prntInvalid)
        ? cntrlValRslt || prntInvalid
        : cntrlValRslt;
    }
  ...
  contCustValErrorToString(formGroup: FormGroup, cntrlName: string): string {
    ...
    let allCustValErrorsForControl = this.getControlValidationErrors(
      formGroup,
      cntrlName
    );
  ...
  getControlValidationErrors(
    formmGroup: FormGroup,
    controlName: string
  ): ICustomValidationError[] {
    let allErrors: CustomValidationError[] =
      this.getCustomFormValidationErrors(formmGroup);

    if (allErrors.length < 0) {
      return [];
    }

    let controlErrors = allErrors.filter(
      (error) => error.control === controlName || error.error === controlName
    );
    ...
    Object.keys(formGroup.controls).forEach((conKey) => {
      ...
      if (control?.errors) {
        Object.keys(control?.errors).forEach((errorKey) => {
          ...
          const error = getError(control?.errors![errorKey]);

          collAllErrors.push(
            new CustomValidationError(conKey, errorKey, error)
          );
        });
      }
      ...
      if (control instanceof FormGroup) {
        ...
        const conInnerErrors: CustomValidationError[] =
          this.getCustomFormValidationErrors(conAsAbsCon);
