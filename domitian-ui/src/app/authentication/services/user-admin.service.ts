...
import {
  LoginResponse,
  loginResponseSchema,
} from '../models/contracts/responses/Ilogin-response';
...
export class AuthenticationService {
  ...
  get<TData>(callbackUrl: string): Observable<TData> {
    let response = this.httpClient
      .get<TData>(callbackUrl, { responseType: 'json' })
  ...
  getConfirmRegistration(email: string): Observable<HttpResponse<string>> {
    let response = this.httpClient
      .post<HttpResponse<string>>(
        this.urlPathService.confirmRegistration(),
        email,
        { responseType: 'json' }
      )
      .pipe(catchError((err: HttpErrorResponse) => this.handleError(err)));
  ...
  register(request: RegisterRequest): Observable<string> {
    let response = this.httpClient
      .post(this.urlPathService.register(), request, { responseType: 'text' })
      .pipe(catchError((err: HttpErrorResponse) => this.handleError(err)));
  ...
  login(request: LoginRequest): Observable<LoginResponse> {
    let response = this.httpClient
      .post<LoginResponse>(this.urlPathService.login(), request, {
        responseType: 'json',
      })
      .pipe(
        tap({
          next: (resp: LoginResponse) => {
            ...
            if (lr.success) this.saveTokens(resp);
          },
  ...
  refreshAccess(): Observable<LoginResponse> {
    let response = this.httpClient
      .post<LoginResponse>(this.urlPathService.refreshAccess(), {
        responseType: 'json',
      })
      .pipe(
        tap({
  ...
  logout(): Observable<HttpResponse<LoginResponse>> {
    let response = this.httpClient
      .post<HttpResponse<LoginResponse>>(this.urlPathService.revokeAccess(), {
        responseType: 'json',
      })
      .pipe(
        tap({
